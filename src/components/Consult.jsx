...
export default function Consult() {
  ...
  return (
    <section
      id="consult"
      className="relative overflow-hidden py-24 px-[5%] bg-[#f6faf0]"
    >
      {/* bg image */}
      <div
      ...
      <div
        ...
        style={{
          backgroundImage:
            "repeating-linear-gradient(45deg, rgba(26,107,47,0.05) 0, rgba(26,107,47,0.05) 1px, transparent 1px, transparent 40px), repeating-linear-gradient(-45deg, rgba(26,107,47,0.05) 0, rgba(26,107,47,0.05) 1px, transparent 1px, transparent 40px)",
        }}
      />
      ...
      <div className="relative z-10 grid lg:grid-cols-2 gap-16 items-start max-w-6xl mx-auto">
        ...
        <div>
          ...
          <h2 className="font-bebas text-4xl lg:text-5xl text-[#0a0f06] leading-none mb-5">
          ...
          <div className="flex flex-col gap-3">
            {contacts.map(({ icon, label, value }) => (
              <div
                key={label}
                className="flex items-center gap-4 p-4 border border-[#1a6b2f]/10 bg-white hover:border-[#1a6b2f]/40 transition-colors"
              >
                <div className="w-9 h-9 flex items-center justify-center bg-[#edf5e1] flex-shrink-0 text-base">
                  {icon}
                </div>
                <div>
                  <p className="font-inter text-[0.68rem] uppercase tracking-wider text-[#5a6b4a]">
                    {label}
                  </p>
                  <p className="font-inter text-sm font-bold text-[#0a0f06] mt-0.5">
                    {value}
                  </p>
                </div>
              </div>
        ...
        {/* RIGHT — FORM */}
        <div className="bg-white border border-[#1a6b2f]/15 shadow-[0_8px_32px_rgba(0,0,0,0.06)] p-10">
          <h3 className="font-bebas text-2xl text-[#0a0f06] mb-1">Request A Consultation</h3>
          <p className="font-inter text-sm text-[#5a6b4a] mb-6">
            We'll reach out on WhatsApp within minutes.
          </p>

          <div className="grid md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block font-inter text-[0.68rem] font-bold uppercase tracking-wider text-[#5a6b4a] mb-1.5">
                Your Name
              </label>
              <input
                name="name"
          ...
          <button
            ...
            className={`w-full mt-4 flex items-center justify-center gap-2 py-3.5 text-sm font-bold uppercase tracking-wider transition-colors ${
              cooldown > 0
                ? "bg-[#25D366]/50 text-white cursor-not-allowed"
